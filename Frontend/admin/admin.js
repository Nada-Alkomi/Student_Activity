...
(function checkAccess() {
  if (!getToken() || getRole().toLowerCase() !== 'admin') {
...
document.querySelectorAll('.sidebar-nav a').forEach(function(link) {
  link.addEventListener('click', function(e) {
    ...
    else if (s === 'activityregs') loadActivityRegistrations();
    else if (s === 'categories') loadCategories();
    else if (s === 'clubs') loadClubs();
    else if (s === 'clubregs') loadClubRegistrations();
    else if (s === 'competitions') loadCompetitions();
    else if (s === 'competitionregs') loadCompetitionRegistrations();
...
async function loadActivityRegistrations() {
  try {
    ...
    if (!data.length) {
      tb.innerHTML = '<tr><td colspan="6"><div class="empty-state"><i class="fa-solid fa-clipboard-list"></i><p>No activity registrations found</p></div></td></tr>';
      return;
    }
...
async function loadCompetitionRegistrations() {
  try {
    ...
    if (!data.length) {
      tb.innerHTML = '<tr><td colspan="6"><div class="empty-state"><i class="fa-solid fa-address-book"></i><p>No competition registrations found</p></div></td></tr>';
      return;
    }
    tb.innerHTML = data.map(function(r) {
      return '<tr>' +
...
async function loadClubRegistrations() {
  try {
    ...
    if (!data.length) {
      tb.innerHTML = '<tr><td colspan="6"><div class="empty-state"><i class="fa-solid fa-users"></i><p>No club registrations found</p></div></td></tr>';
      return;
    }
    tb.innerHTML = data.map(function(r) {
      return '<tr>' +
